...
for (const [lessonId, words] of Object.entries(extra)) {
  const filePath = path.join(__dirname, '../src/data/lessons', `lesson${lessonId}.ts`);
  ...
  if (wordMatches) {
    exWords = wordMatches.map(m => m.match(/"word":\s*"([^"]+)"/)[1]);
  }
  
  // Find the end of the words array
  // Wait, in the modularized file, it's JSON.stringify'd!
  // It looks like:
  // "words": [
  //   {
  //     "id": "L4_01",
  ...
  //   }
  // ],
  // Let's parse it properly instead of regex!
  // Wait, it's a TS file with `export default lesson;`. We can just import it, append words, and write it back.
}
